...
calendar read), and `POST /api/events` (create an event — always `DRAFT`,
with a timezone-aware planned time). Interactive docs (Swagger UI) at
...
`POST /api/persons`, `GET /api/events` (calendar-oriented list), and
