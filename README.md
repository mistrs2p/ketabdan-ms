...
roles), and `POST /api/persons` (create a branch member, optionally with
roles — the first write endpoint). Interactive docs (Swagger UI) at
...
**First write endpoint stage.** The repository contains the runnable
...
first business endpoints — `GET /api/roles`, `GET /api/persons`, and
`POST /api/persons` (person creation with atomic role memberships,
[docs/06](docs/06-BACKEND-API.md) §4a). The remaining business APIs and UI
features follow as the open domain questions
